...
const BeerProfile = () => {
    ...
    const loadData = async () => {
        ...
        setBeer({data: response.data, averages: {rating: avRating.toFixed(1), body: avBody.toFixed(1), linger: avLinger.toFixed(1), herbal: avHerbal.toFixed(1), citrus: avCitrus.toFixed(1), hoppy: avHoppy.toFixed(1), bitter: avBitter.toFixed(1), sour: avSour.toFixed(1)}});
        // } else {
    ...
    return (beer && 
        ...
        <p>Brewery: <Link to={`/breweries/${beer.data.brewery.id}`}>{beer.data.brewery.user.brewery_name}</Link></p>
        <p>Brewery Picture:</p>
